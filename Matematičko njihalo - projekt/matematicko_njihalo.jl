...
# ╔═╡ 11d96e10-1dab-11ef-3e89-dd3dd00d81d3
using PlutoUI, Plots, DifferentialEquations;

# ╔═╡ 3b37f5a2-6bec-455d-a775-3222fc74f469
md"# Matematičko njihalo"

# ╔═╡ d7a81cd1-5d36-4a02-b12f-fa6f6ffe4d97
md"U ovom projektu razrađuje se matematičko njihalo nad kojim djeluje otpor zraka."

# ╔═╡ 2f4a3554-8475-4734-8e10-d74ae3a97998
md"![](https://cdn.britannica.com/71/2471-050-F6EB87D5/pendulum.jpg)"

# ╔═╡ b7c5325e-7663-4505-9112-e4fbcebce6f4
md"Ishodište se smatra točkom za koje je tijelo obješeno o nit. Tako će $x$ označavati os apscisa, a $y$ os ordinata."

# ╔═╡ 61c69d8a-52f2-4aef-aacf-0bca857e2366
md"Model jednostavnog modela matematičkog njihala bez otpora se rješava preko sljedeće dijefrencijalne jednadžbe:"

# ╔═╡ 0c161fe2-2bab-4f5f-9e52-07ab89a3936f
md"$-mg\sin{\theta} = m\ddot{\theta}l$"

# ╔═╡ 34b47e32-0881-4ff3-94b9-83888ecf8162
md"Međutim, ovdje se želi modelirati matematičko njihalo u sustavu gdje postoji otpor zraka. U tu svrhu uvodi se konstanta $\lambda$ koja se izražava u $kgs^{-1}$ i nova jednadžba poprima sljedeći oblik:"

# ╔═╡ 48e9f8e3-8141-429e-aedb-e06b2d427d0e
md"$-mg\sin{\theta} - \lambda\dot{\theta}l = m\ddot{\theta}l$"

# ╔═╡ 61fa7d56-c0ee-4780-9ac4-e01025c720d4
md"Ovaj model Julijin SecondOrderODEProblem ne može riješiti jer diferencijalna jednadžba nije linearna, a i analitički je nezgrapno riješiti ovakav izraz. Uvode se pojednostavljenja modelu."

# ╔═╡ bf8f110b-3176-4b30-8b86-319c1fd4944f
md"Pretpostavka jest da je $\theta$ mali kut, stoga $\sin{\theta} \approx \theta \implies \theta = \frac{x}{l}$. Algebarskim manipulacijama se dobije sljedeća jednadžba na kojoj se model temelji:" 

# ╔═╡ a3043fdf-10cb-45fe-b656-f3d1dc988f43
md"$\ddot{x} = -\frac{\lambda}{m}\dot{x} - \frac{g}{l}x$" 

# ╔═╡ ec16a5fe-9ef2-4d2a-8977-0f3306c027da
md"# Graf gibanja matematičkog njihala" 

# ╔═╡ cd980b60-06a8-447a-9603-0b3bc3c2a6ad
md"Sljedeći parametri su varijabilni koristeći slidere. Jedino što je konstantno je $g$ jer je pretpostavka da smo na Zemlji. Masa je izražena u kilogramima, duljina u metrima, a lambda u $kgs^{-1}$. Ove varijable mijenjaju samo ovaj graf. Ne mijenjaju animaciju u nastavku."

# ╔═╡ cf3f95ed-167b-4864-a198-f79567fd7e62
begin
	sliderm = @bind m Slider(0.1:0.1:3.0, default=1, show_value=true)
	sliderl = @bind l Slider(0.25:0.25:5.0, default=2, show_value=true)
	sliderlambda = @bind lambda Slider(0.0:0.01:1.0, default=0.3, show_value=true)
...
# ╔═╡ c2e36693-4432-4eba-83e8-8e503419b059
const g = 9.80665

# ╔═╡ 8e8ea525-3070-4ebc-a6fc-4c32aa780b98
p = (m, g, l, lambda)

# ╔═╡ 38a976e9-8bad-4921-ba7a-295b059c4e84
u0 = [0.0] #pocetni polozaj x

# ╔═╡ 691f4a78-a30c-41bd-aad1-2cc85c6028a5
 du0 = [pi/2] #pocetna brzina

# ╔═╡ 982b2832-3f27-47ce-a9d1-74ba3cda9963
tspan = (0.0, 6pi)

# ╔═╡ 1fce31e8-b0e4-4d01-9f6c-6ec0afceb1e4
matematicko_njihalo = function(ddu, du, u, p, t) #fja za graf
	ddu .= -(lambda / m) * du - (g / l) * u
end

# ╔═╡ 775fdff2-fc2a-4799-b81e-f0aac17f26ae
problem = SecondOrderODEProblem(matematicko_njihalo, du0, u0, tspan)

# ╔═╡ b2503c31-ab52-42b3-a0af-c7e6e2a9ca2a
sol = solve(problem)

# ╔═╡ 808c7730-4349-404b-9617-345ac0cfa07d
plot(sol, idxs = [2, 1], linewidth = 2, title = "Matematičko njihalo", xlabel = "Vrijeme [s]", ylabel = "x [m] / v [m/s]", label = ["Položaj" "Brzina"])

# ╔═╡ fdfa9f4b-f021-4d16-bd5c-141b96803164
md"# Animacija gibanja matematičkog njihala"

# ╔═╡ 2652bc7d-68eb-45c4-99b5-e41a555bc20e
md"Parametri su varijabilni kao i u prethodnom primjeru s istim mjernim jedinicama. Ove varijable su vezane samo uz ovu animaciju, ne mijenjaju prethodni graf."

# ╔═╡ 27578c94-eeb8-4bf9-8d3c-57968acf970f
begin
	sliderm1 = @bind m1 Slider(0.1:0.1:3.0, default=1, show_value=true)
	sliderl1 = @bind l1 Slider(0.25:0.25:5.0, default=2, show_value=true)
	sliderlambda1 = @bind lambda1 Slider(0.0:0.01:1.0, default=0.3, show_value=true)
	slidertime = @bind time Slider(0.0:0.5:50, default=0.0, show_value=true)
	
	md"Masa(m): $(sliderm1) \
	Duljina(l): $(sliderl1) \
	Lambda: $(sliderlambda1)\
	Vrijeme: $(slidertime)"
end

# ╔═╡ a33d3009-8b2d-48c4-ab85-81020580dddd
p1 = (m1, g, l1, lambda1)

# ╔═╡ dfd9aa08-a8de-45c4-bd7a-171a676d3d1d
tspan1 = (0.0, 50.0)

# ╔═╡ 5d51f63d-268d-4760-9b78-be8a54107f37
matematicko_njihalo1 = function(ddu, du, u, p, t) #fja za animaciju
	ddu .= -(lambda1 / m1) * du - (g / l1) * u
end

# ╔═╡ fdfa3381-c1f8-4ab7-a4d6-9ec8643b7e70
problem1 = SecondOrderODEProblem(matematicko_njihalo1, du0, u0, tspan1, p1)

# ╔═╡ 0cd478a6-7680-4f4f-b594-a5c4b0e73829
sol1 = solve(problem1)

# ╔═╡ 38158047-91de-4973-8afa-1c5f29b1184c
x_t = sol1(time)[2] #spremanje x-vrijednosti iz rjesenja jednadzbe

# ╔═╡ 67fb28d3-498f-4683-9be9-207b563f3202
function pozicija_njihala(x_t, l1)
	theta = x_t / l1              #aproksimacija s pocetka sin(theta) = theta
	pozicija_y = -l1 * cos(theta) #racunanje pozicije y
	return x_t, pozicija_y
end

# ╔═╡ b6952933-53bf-4b54-931c-4ab52fc17563
x, y = pozicija_njihala(x_t, l1)

# ╔═╡ 0fcfd79f-42d9-4427-90a8-a57a5262a7a8
begin
	#dodavanje vektora (niti)
	plot([0, x], [0, y], seriestype =:path, arrow =:arrow, linewidth = 1, title = "Animacija njihala", xlabel = "x [m]", ylabel = "y [m]", legend = false, color =:black)
	#dodavanje tocke (kugla)
	plot!([x], [y], seriestype =:scatter, color=:red, markersize=10) 
	#prilagodba ravnine
	plot!(size = (500, 500), xlims = (-2, 2), ylims = (-6, 0))
end

# ╔═╡ b1becf17-bf80-49f2-b932-e840b0bb1c8d
md"Kompletan model temelji se na saznanjima sa sljedećeg [izvora](https://nrich.maths.org/6478/solution)." 
